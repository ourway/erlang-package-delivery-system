...
handle_info({package_delivered, Package}, State) ->
	...
	NewState = maps:update("assignments", 
				maps:get("assignments", State) -- DeliveredPackages, State),
...
handle_info({'DOWN', _Ref, process, Deliverator, Reason}, State) ->
	...
	FailedAssignments = filter(
			...
			maps:get("assignments", State)),
	FailedPackages = map(fun(A) -> 
			maps:remove("deliverator", A) end, FailedAssignments),
...
assign_packages(State, Packages, Deliverator) ->
	NewAssignments = map(
			fun(P) -> maps:put("deliverator", Deliverator, P) end, Packages),
