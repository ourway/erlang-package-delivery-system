...
%%%
%%% @end
%%% Created : 2021-12-01 02:52:11.631873
%%%-------------------------------------------------------------------
...
-record(state, {sock, client, server}).
...
%%--------------------------------------------------------------------
start_link() ->
    {ok, Sock} = gen_tcp:listen(9000, [{packet, 0}, {active, false}, {reuseaddr, true}]),
...
%% @end
%%--------------------------------------------------------------------
handle_cast(accept, #state{sock = Listen} = State) ->
    Pid = spawn_link(?MODULE, accept, [Listen]),
    monitor(process, Pid),
    {noreply, State}.
